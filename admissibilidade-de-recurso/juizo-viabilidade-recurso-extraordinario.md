...
5. **Temas.** No trecho "Tema(s):", copie cada identificador que vem depois de "ID:", sem asteriscos nem parênteses (ex.: "(ID: **stf-rg-123**)" → stf-rg-123). Copie o identificador só desse trecho: não o procure nas partes anteriores da pesquisa nem o monte a partir do número do tema. "não se aplica" significa nenhum tema.
...
### 3.3 Óbice específico do recurso extraordinário → motivo
...
- Ausência de preliminar de repercussão geral → AUSENCIA_PRELIMINAR_REPERCUSSAO_GERAL
- Reexame fático-probatório → FATICA_PROBATORIA
- Interpretação de cláusula contratual → CLAUSULA_CONTRATUAL
- Ofensa reflexa à Constituição → OFENSA_REFLEXA
- Direito local → DIREITO_LOCAL
- Matéria regimental → MATERIA_REGIMENTAL
- Decisão sobre liminar ou tutela provisória → DECISAO_LIMINAR_TUTELA_PROVISORIA
- Conformidade com a jurisprudência do STF → CONFORMIDADE_JURISPRUDENCIA
...
7. **Identificador de tema inválido.** Só transcreva identificador formado por "stf-rg-" seguido apenas de algarismos. Identificador de tema do STJ ("stj-rr-") ou em outro formato: não o transcreva e registre "[VERIFICAR] Pedido [número]: identificador de tema inválido para recurso extraordinário ([identificador])." Se o pedido ficar sem tema, aplique também o item 3.
...
- **Pedido 1:** negar seguimento — **Tema(s):** Tema de Repercussão Geral Nº 0000 (ID: **stf-rg-0000**) — **Óbices:** não se aplica.
- **Pedido 2:** inadmitir — **Tema(s):** não se aplica — **Óbices:** Ausência de prequestionamento (Súmulas 282 e 356/STF); Reexame fático-probatório (Súmula 279/STF).
...
- motivoGeral: [].
- Pedido 1: dispositivo NEGAR_SEGUIMENTO; tema [stf-rg-0000]; motivo []. Argumentos: iguais ao pedido.
...
- O identificador tem o formato "stf-rg-456" e é copiado só do trecho "Tema(s)" da linha deste pedido na Proposta de solução. Não o procure nas partes anteriores da pesquisa nem o monte a partir do número do tema.
- Identificadores no formato "stj-rr-NNN" referem-se a temas de recursos repetitivos do STJ e não devem ser utilizados no juízo de viabilidade do recurso extraordinário. Informe apenas identificadores de temas de repercussão geral do STF.
